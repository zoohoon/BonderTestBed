namespace GPLoaderRouter.RobotCommands
{
    ...
    public class Cassette_Pick : GPLoaderRobotCommand
    {
        public Cassette_Pick()
        {
            CommandName = EnumRobotCommand.CST_PICK;
            ...
            try
            {
                switch (SystemManager.SystemType)
                {
                    ...
                    case SystemTypeEnum.GOP:
                        ...
                        CommandSequence.Add(20, new List<SequenceBehavior>() { new Check_Foup_HomingState(),
                                                                               new Move_PickPos_LX_LZ_LW() });
                        CommandSequence.Add(30, new List<SequenceBehavior>() { new Move_TargetPos_Arm() });              // (6, 8 inch) CST에서는 웨이퍼를 들고 나서 Arm을 더 뻗어서 튀어나오지 않게 하기 위함.
                        CommandSequence.Add(32, new List<SequenceBehavior>() { new Move_Up_LZ() });                      // (6, 8 inch) CST에서는 웨이퍼를 들고 나서 Arm을 더 뻗어서 튀어나오지 않게 하기 위함.
                        CommandSequence.Add(34, new List<SequenceBehavior>() { new Move_TargetPos_Arm(), 
                                                                               new Vac_On_Arm() });
                        CommandSequence.Add(40, new List<SequenceBehavior>() { new Move_Up_LZ(), 
                                                                               new CheckVac_Arm_AfterPick() });
                        CommandSequence.Add(60, new List<SequenceBehavior>() { new Move_BackPos_Arm() });
