namespace GPLoaderRouter.FoupCommands
{
    ...
    public class Foup_CoverLock : GPLoaderCSTCommand
    {
        public Foup_CoverLock(SubstrateSizeEnum waferSize)
        {
            CommandName = EnumCSTCtrl.COVERLOCK;
            WaferSize = waferSize;
            ...
            try
            {
                ...
                if (SystemManager.SystemType == SystemTypeEnum.Opera)
                {
                    if (WaferSize == SubstrateSizeEnum.INCH12)
                    {
                        ...
                        CommandSequence.Add(5, new List<SequenceBehavior>() { new CheckNoWaferOut(), new InitCoverLockingState() });
                        CommandSequence.Add(10, new List<SequenceBehavior>() { new CheckCstVacOn(), new CstVacOff(), new CstMappingOff() });
                        CommandSequence.Add(15, new List<SequenceBehavior>() { new CheckCstMappingOff() });
                        CommandSequence.Add(20, new List<SequenceBehavior>() { new CoverUnlockOn() });
                        CommandSequence.Add(25, new List<SequenceBehavior>() { new CheckCoverUnlock() });
                        ...
                        CommandSequence.Add(60, new List<SequenceBehavior>() { new CheckCstVacOn(), new CoverLockOn() });
                        CommandSequence.Add(62, new List<SequenceBehavior>() { new CoverLockOn() });
                        CommandSequence.Add(70, new List<SequenceBehavior>() { new CheckCoverLock(), new CstVacOff() });
                        CommandSequence.Add(80, new List<SequenceBehavior>() { new CheckCstVacOff() });
                        CommandSequence.Add(100, new List<SequenceBehavior>() { new InitCoverLockedState() });
                    }
                    else
                    {
                        //not performed cover lock.
                    }
                ...
                else if (SystemManager.SystemType == SystemTypeEnum.GOP)
                {
                    if (WaferSize == SubstrateSizeEnum.INCH12)
                    {
                        CommandSequence.Add(0, new List<SequenceBehavior>() { new InitCoverLockingState(), new CstVacOn() });
                        CommandSequence.Add(5, new List<SequenceBehavior>() { new CheckNoWaferOut(), new InitCoverLockingState() });
                        ...
                        CommandSequence.Add(27, new List<SequenceBehavior>() { new CheckCoverOpen() });
                        CommandSequence.Add(30, new List<SequenceBehavior>() { new MoveDownPos(), new CoverCloseOn() });
                        CommandSequence.Add(40, new List<SequenceBehavior>() { new CheckCoverLock() });
                        CommandSequence.Add(50, new List<SequenceBehavior>() { new CstVacOn() });
                        CommandSequence.Add(60, new List<SequenceBehavior>() { new CheckCstVacOn(), new CoverLockOn() });
                        CommandSequence.Add(62, new List<SequenceBehavior>() { new CoverLockOn() });
                        CommandSequence.Add(70, new List<SequenceBehavior>() { new CstVacOff() });
                        CommandSequence.Add(80, new List<SequenceBehavior>() { new CheckCstVacOff() });
                        CommandSequence.Add(100, new List<SequenceBehavior>() { new InitCoverLockedState() });
                    }
                    else
                    ...
                    //6,8인치는 command 받았을 때 무조건 cover locked로 처리.
                }
