namespace LogModule.LoggerRule
{
    ...
    using NLog.Config;
    using NLog.Targets;
    public class PMILoggerRuleConfig : NLoggerRuleConfiger
    {
        ...
        {
            //==> File rule setting
            LoggerFileTarget = new FileTarget("PMILoggerFileTarget");
            ...
            LoggerFileTarget.FileName = $"{param.LogDirPath}" + "\\PMI_${shortdate}.log";
            ...
            LoggerFileTarget.ArchiveFileName = "C:\\Logs\\PMI\\Archives\\PMI_${date:format=yyyy-MM-dd HHmmss}.log";
            ...
            config.AddTarget("PMILogFile", LoggerFileTarget);
            LoggingRule PMILogFileRule = new LoggingRule(loggerName, LogLevel.Debug, LogLevel.Info, LoggerFileTarget);
            config.LoggingRules.Add(PMILogFileRule);

            ////==> Console rule setting
            //ConsoleTarget PMILogConsoleTarget = new ConsoleTarget();
            ////debugLogConsoleTarget.Layout = "${date:format=yyyy-MM-dd HH\\:mm\\:ss.fff} | [D] | ${message}";
            //PMILogConsoleTarget.Layout = "${date:format=yyyy-MM-dd HH\\:mm\\:ss.fff} | ${message}";
            //config.AddTarget("PMILogConsole", PMILogConsoleTarget);
            //LoggingRule PMILogConsoleRule = new LoggingRule(loggerName, LogLevel.Debug, PMILogConsoleTarget);
            //config.LoggingRules.Add(PMILogConsoleRule);
