namespace LogModule.LoggerRule
{
    ...
    using NLog.Config;
    using NLog.Targets;
    public class PinLoggerRuleConfig : NLoggerRuleConfiger
    {
        ...
        {
            //==> File rule setting
            LoggerFileTarget = new FileTarget("PinLoggerFileTarget");
            ...
            LoggerFileTarget.FileName = $"{param.LogDirPath}" + "\\Pin_${shortdate}.log";
            ...
            LoggerFileTarget.ArchiveFileName = "C:\\Logs\\Pin\\Archives\\Pin_${date:format=yyyy-MM-dd HHmmss}.log";
            ...
            config.AddTarget("PinLogFile", LoggerFileTarget);
            LoggingRule PinLogFileRule = new LoggingRule(loggerName, LogLevel.Debug, LogLevel.Info, LoggerFileTarget);
            config.LoggingRules.Add(PinLogFileRule);

            ////==> Console rule setting
            //ConsoleTarget PinLogConsoleTarget = new ConsoleTarget();
            ////debugLogConsoleTarget.Layout = "${date:format=yyyy-MM-dd HH\\:mm\\:ss.fff} | [D] | ${message}";
            //PinLogConsoleTarget.Layout = "${date:format=yyyy-MM-dd HH\\:mm\\:ss.fff} | ${message}";
            //config.AddTarget("PinLogConsole", PinLogConsoleTarget);
            //LoggingRule PinLogConsoleRule = new LoggingRule(loggerName, LogLevel.Debug, PinLogConsoleTarget);
            //config.LoggingRules.Add(PinLogConsoleRule);
