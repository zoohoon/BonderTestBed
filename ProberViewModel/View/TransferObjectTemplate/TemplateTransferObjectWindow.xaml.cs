...
namespace ProberViewModel
{
    ...
    /// </summary>
    public partial class TemplateTransferObjectWindow : Window, INotifyPropertyChanged
    {
        ...
        #endregion
        public TemplateTransferObjectWindow()
        {
            InitializeComponent();
        }
    }

    public class TransferObjectSelectDataTemplate : DataTemplateSelector
    {
        public override DataTemplate SelectTemplate(object item, DependencyObject container)
        {
            ...
            try
            {
                if (item != null)
                {
                    TransferObject to = item as TransferObject;
                    FrameworkElement el = container as FrameworkElement;

                    switch (to.WaferType.Value)
                    {
                        ...
                        case EnumWaferType.STANDARD:
                            retval = (DataTemplate)el.FindResource("Standard");

                            break;
                        case EnumWaferType.POLISH:
                            retval = (DataTemplate)el.FindResource("Polish");
                            break;
                        case EnumWaferType.CARD:
                            //retval = (DataTemplate)el.FindResource("DataTemplate3");
                            break;
                        default:
                            break;
                    }
