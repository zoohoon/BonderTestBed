...
namespace AlignKeyBlob.Tests
{
    [TestClass()]
    public class SinglePinAlignKeyBlobModuleTests : TestBaseProberSystem
    {
        /// <summary>
        /// PinHighAlignModule.cs에서 동작하는 SinglePinalign()을 기준으로 제작하였음.
        /// </summary>
        [TestMethod()]
        public void TipBlobModuleTest()
        {
        ...
        [TestMethod()]
        public void KeyBlobTest()
        {
            Container.Resolve<IFileManager>(new NamedParameter("ParamPath", "C:\\ProberSystem\\Emul_c01"));
            ...
            string folderPath = @"C:\Logs\Test\rer";
            ...
            foreach (FileInfo filePath in files)
            {
                ...
                // (3) FindBlobWithRectangularity()
                modulename = "VisionManager";       // Otsu
                path = filePath.FullName;
                this.VisionManager().SetParameter(camtype, modulename, path);
                //(3-1)
                modulename = "VisionManager";       // Adaptive Binarize
                path = filePath.FullName;
                this.VisionManager().SetParameter(camtype, modulename, path);

                // (3-2) ModelFind()
                modulename = "SinglePinAlignKeyBlobModule";
                path = filePath.FullName;
                this.VisionManager().SetParameter(camtype, modulename, path);

                var result = this.PinAligner().SinglePinAligner.SinglePinalign(
                                    out NewPinPos,
                                    this.StageSupervisor().ProbeCardInfo.ProbeCardDevObjectRef.DutList[CurDutIndex].PinList[CurPinArrayIndex],
                                    PinFocusModel,
                                    FocusParam);
                index++;
                System.Diagnostics.Debug.WriteLine($"#{index:000} File Name = {path}, result = {result}");
