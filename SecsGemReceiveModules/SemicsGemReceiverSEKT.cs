...
namespace SecsGemReceiveModules
{
    ...
    public class SemicsGemReceiverSEKT : ISecsGemMessageReceiver
    {
        ...
        private void XGEM_OnSECSMessageReceive_S2F4X(long nMsgId, long nStream, long nFunction, long nSysbyte)
        {
            try
            {
                ...
                if (REMOTE_COMMAND_ACTION.ToUpper() == EnumRemoteCommand.PSTART.ToString())
                {
                    ...
                    Gem.GetAscii(nMsgId, ref foupnumber);// [U] PTN
                    actreqdata.FoupNumber = Convert.ToInt32(foupnumber);
                ...
                else if(REMOTE_COMMAND_ACTION.ToUpper() == EnumRemoteCommand.UNDOCK.ToString())
                {
                    ...
                    var actreqdata = new UnDockReqData();
                    actreqdata.ObjectID = pnObjectID;
                    actreqdata.Stream = nStream;
                    actreqdata.Function = nFunction;
                    actreqdata.Sysbyte = nSysbyte;
                    ...
                    Gem.GetList(nMsgId, ref dataListCount);// [L] PortNo.
                    Gem.GetAscii(nMsgId, ref CPNAME);// [A] PortNumber
                    Gem.GetU1(nMsgId, ref PTN);// [A] PortNumber

                    actreqdata.LoadPortNumber = PTN[0]; //Data [U] PortNumber
