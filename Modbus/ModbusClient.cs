...
namespace EasyModbus
{
    ...
    /// </summary>
    public partial class ModbusClient : ICommunicationMeans
    {
        ...
        public void Connect(string ipAddress, int port)
        {
            try
            {
                if (!udpFlag)
                {
                    ...
                    if (!success)
                    {
                        var retry = result.AsyncWaitHandle.WaitOne(connectTimeout);
                        if (!retry)
                            throw new EasyModbus.Exceptions.ConnectionException("connection timed out");
                    }
                    tcpClient.EndConnect(result);
        ...
        public int[] ReadHoldingRegisters(int startingAddress, int quantity, int waitTimeout = 0)
        {
            int[] response;
            try
            {
                if (debug)
                {
                    StoreLogData.Instance.Store("FC3 (Read Holding Registers from Master device), StartingAddress: " + startingAddress + ", Quantity: " + quantity, System.DateTime.Now);
                }
                ...
                if (serialport != null)
                {
                    if (!serialport.IsOpen)
                    {
                        if (debug)
                        {
                            StoreLogData.Instance.Store("SerialPortNotOpenedException Throwed", System.DateTime.Now);
                        }
                        throw new EasyModbus.Exceptions.SerialPortNotOpenedException("serial port not opened");
                    }
                }
                ...
                if (startingAddress > 65535 | quantity > 125)
                {
                    if (debug)
                    {
                        StoreLogData.Instance.Store("ArgumentException Throwed", System.DateTime.Now);
                    }
                ...
                this.startingAddress = BitConverter.GetBytes(startingAddress);
                //this.startingAddress[1] = SubModuleIndex;
                this.quantity = BitConverter.GetBytes(quantity);
                ...
                {
                    LoggerManager.Error($"Modbus Client is not connected. (serialport : {serialport}, tcpClient.Client.Connected : {tcpClient.Client.Connected}, udpFlag : {udpFlag}");
                    return null;
                }
        ...
        public bool WriteSingleRegister(int startingAddress, int value, int waitTimeout = 0)
        {
            bool retVal = false;
            try
            {
        ...
        private byte _SubModuleIndex;

        public byte SubModuleIndex
        {
