...
{
    internal class SemicsOCRModule : AttachedModuleBase, ISemicsOCRModule
    {
        public override bool Initialized { get; set; } = false;
        public override ModuleTypeEnum ModuleType => ModuleTypeEnum.SEMICSOCR;

        public OCRTypeEnum OCRType => OCRTypeEnum.SEMICS;

        public OCRDefinitionBase OCRDefinitionBase => Definition;

        public SemicsOCRDefinition Definition { get; set; }
        
        public SemicsOCRDevice Device { get; set; }
        public ReservationInfo ReservationInfo { get; set; }

        public EventCodeEnum SetDefinition(SemicsOCRDefinition definition, int index)
        {
        ...
        public EventCodeEnum SetDevice(SemicsOCRDevice device)
        {
            EventCodeEnum retVal = EventCodeEnum.UNDEFINED;
        ...
        public IPreAlignModule GetDependecyPA()
        {
            ...
            try
            {
                PA = Loader.ModuleManager.FindModule(ModuleTypeEnum.PA, Definition.DependencyPreAlignNum.Value) as IPreAlignModule;
            }
            catch (Exception err)
            {
                LoggerManager.Exception(err);
        ...
        public SubchuckMotionParam GetSubchuckMotionParam(SubstrateSizeEnum size)
        {
            ...
            try
            {
                param = Definition.SubchuckMotionParams
               .Where(
               item =>
               item.SubstrateSize.Value == size
               ).FirstOrDefault();
