using System;
using System.Threading.Tasks;

namespace CoordinateSystem.CameraPositionResolver
{
    using ProberInterfaces;
    using ProberInterfaces.Param;
    using ProberErrorCode;
    using LogModule;

    public class WaferHighMove : CameraMoveBase
    {
        public override bool Initialized { get; set; } = false;

        public WaferHighMove()
        {

        }
        public override EventCodeEnum InitModule()
        {
            EventCodeEnum retval = EventCodeEnum.UNDEFINED;

            try
            {
                if (Initialized == false)
                {
                    ZeroPosition.X.Value = this.CoordinateManager().StageCoord.RefMarkPos.X.Value
                        - this.CoordinateManager().StageCoord.MarkPosInChuckCoord.X.Value;
                    ZeroPosition.Y.Value = this.CoordinateManager().StageCoord.RefMarkPos.Y.Value
                        - this.CoordinateManager().StageCoord.MarkPosInChuckCoord.Y.Value;
                    ZeroPosition.Z.Value = this.CoordinateManager().StageCoord.RefMarkPos.Z.Value
                        - this.CoordinateManager().StageCoord.MarkPosInChuckCoord.Z.Value;

                    Initialized = true;

                    retval = EventCodeEnum.NONE;
                }
                else
                {
                    LoggerManager.Error($"DUPLICATE_INVOCATION IN {this.GetType().Name}");

                    retval = EventCodeEnum.DUPLICATE_INVOCATION;
                }
            }
            catch (Exception err)
            {
                LoggerManager.Exception(err);
            }

            //ZeroPosition.X.Value += this.CoordinateManager().StageCoord.WHOffset.X.Value;
            //ZeroPosition.Y.Value += this.CoordinateManager().StageCoord.WHOffset.Y.Value;
            //ZeroPosition.Z.Value += this.CoordinateManager().StageCoord.WHOffset.Z.Value;

            //- this.CoordinateManager().StageCoord.MarkPosInDiskPosCoord.X.Value;

            return retval;
        }

        public void Init()
        {
        }

        public async override Task<int> MoveAsync(
            double xpos = 0, double ypos = 0, double zpos = 0, double tpos = 0)
        {
            double zclearence = -10000;
            try
            {
                double zActualpos = 0.0;

                var axisX = this.MotionManager().GetAxis(EnumAxisConstants.X);
                var axisY = this.MotionManager().GetAxis(EnumAxisConstants.Y);
                var axisZ = this.MotionManager().GetAxis(EnumAxisConstants.Z);
                WaferCoordinate waferhigh = new WaferCoordinate();
                MachineCoordinate machine = new MachineCoordinate();

                waferhigh.X.Value = xpos;
                waferhigh.Y.Value = ypos;
                waferhigh.Z.Value = zpos * -1;

                //zpos = zpos * -1d;
                //zpos += this.CoordinateManager().StageCoord.WHOffset.Z.Value;
                //xpos -= this.CoordinateManager().StageCoord.WHOffset.X.Value;
                //ypos += this.CoordinateManager().StageCoord.WHOffset.Y.Value;

                machine = this.CoordinateManager().WaferHighChuckConvert.ConvertBack(waferhigh);

                //WaferAligner.HeightSearchIndex(xpos, ypos);
                //zpos = WaferAligner.GetHeightValue(xpos, ypos);

                this.MotionManager().GetActualPos(EnumAxisConstants.Z, ref zActualpos);
                if (zclearence < zActualpos)
                {
                    this.MotionManager().AbsMove(axisZ,
                    zclearence,
                    axisZ.Param.Speed.Value,
                    axisZ.Param.Acceleration.Value);
                }

                this.MotionManager().AbsMove(
                       axisZ,
                       machine.Z.Value,
                       axisZ.Param.Speed.Value,
                    axisZ.Param.Acceleration.Value);

                this.MotionManager().AbsMove(
                    axisX,
                    machine.X.Value,
                    axisX.Param.Speed.Value,
                    axisX.Param.Acceleration.Value);

                this.MotionManager().AbsMove(
                    axisY,
                    machine.Y.Value,
                    axisY.Param.Speed.Value,
                    axisY.Param.Acceleration.Value);

                await this.MotionManager().WaitForMotionDoneAsync();

            }
            catch (Exception err)
            {
                LoggerManager.Exception(err);
                throw;
            }
            return 0;
        }

        public override Task<int> IndexMove(double diesizex, double diesizey, int movex,
            int movey, double squarness = 0, double zpos = 0)
        {
            //double waferthick = 1113;
            //double zclearence = -10000;
            try
            {
                //double zActualpos = 0.0;
                //double xpos = 0.0;
                //double ypos = 0.0;
                //zpos = zpos * -1d;

                ////=======REFDIE 

                //zpos += this.CoordinateManager().StageCoord.WHOffset.Z.Value;
                //xpos += this.CoordinateManager().StageCoord.WHOffset.X.Value + 
                //    WaferAlinger.WaferAlignInfo.WaferCenterX + WaferAlinger.WaferAlignInfo.RefDie.X.Value;
                //ypos += this.CoordinateManager().StageCoord.WHOffset.Y.Value +
                //    WaferAlinger.WaferAlignInfo.WaferCenterY + WaferAlinger.WaferAlignInfo.RefDie.Y.Value;

                //this.MotionManager().GetActualPos(EnumAxisConstants.Z, out zActualpos);
                //if (zclearence < zActualpos)
                //{
                //    this.MotionManager().AbsMove(axisZ,
                //    zclearence,
                //    axisZ.Param.Speed.Value,
                //    axisZ.Param.Acceleration.Value);
                //}

                //this.MotionManager().AbsMove(
                //     axisZ,
                //     zpos,
                //     axisZ.Param.Speed.Value,
                //  axisZ.Param.Acceleration.Value);

                //xpos += movex * diesizex;

                //this.MotionManager().AbsMove(
                //    axisX,
                //    xpos,
                //    axisX.Param.Speed.Value,
                //    axisX.Param.Acceleration.Value);

                //ypos += movey * diesizey;

                //this.MotionManager().AbsMove(
                //    axisY,
                //    ypos,
                //    axisY.Param.Speed.Value,
                //    axisY.Param.Acceleration.Value);

                //await this.MotionManager().WaitForMotionDoneAsync();

                //return 0;
                var axisX = this.MotionManager().GetAxis(EnumAxisConstants.X);
                var axisY = this.MotionManager().GetAxis(EnumAxisConstants.Y);
                var axisZ = this.MotionManager().GetAxis(EnumAxisConstants.Z);
                double zclearence = -10000;
                double zActualpos = 0.0;

                zpos = zpos * -1d;
                double xpos = diesizex * movex;
                double ypos = diesizey * movey;

                double curxpos = 0.0;
                this.MotionManager().GetActualPos(EnumAxisConstants.X, ref curxpos);
                double curypos = 0.0;
                this.MotionManager().GetActualPos(EnumAxisConstants.Y, ref curypos);
                squarness = this.WaferAligner().GetReviseSquarness(curxpos + xpos, curypos + ypos);


                this.MotionManager().GetActualPos(EnumAxisConstants.Z, ref zActualpos);
                if (zclearence < zActualpos)
                {
                    this.MotionManager().AbsMove(axisZ,
                    zclearence,
                    axisZ.Param.Speed.Value,
                    axisZ.Param.Acceleration.Value);
                }

                if (xpos != 0)
                {
                    this.MotionManager().RelMoveAsync(axisX,
                        (xpos),
                        axisX.Param.Speed.Value,
                        axisX.Param.Acceleration.Value);
                }

                if (ypos != 0)
                {
                    this.MotionManager().RelMoveAsync(axisY,
                      (ypos),
                      axisY.Param.Speed.Value,
                      axisY.Param.Acceleration.Value);
                }

            }
            catch (Exception err)
            {
                LoggerManager.Exception(err);
                throw;
            }
            return Task.FromResult<int>(0);
        }

        public override Task<int> RelMove(double diesizex, double diesizey, int movex,
            int movey, double squarness = 0, double zpos = 0)
        {
            double zclearence = -10000;
            try
            {
                double zActualpos = 0.0;
                var axisX = this.MotionManager().GetAxis(EnumAxisConstants.X);
                var axisY = this.MotionManager().GetAxis(EnumAxisConstants.Y);
                var axisZ = this.MotionManager().GetAxis(EnumAxisConstants.Z);
                zpos = zpos * -1d;
                double xpos = diesizex * movex;
                double ypos = diesizey * movey;

                double curxpos = 0.0;
                this.MotionManager().GetActualPos(EnumAxisConstants.X, ref curxpos);
                double curypos = 0.0;
                this.MotionManager().GetActualPos(EnumAxisConstants.Y, ref curypos);

                squarness = this.WaferAligner().GetReviseSquarness(curxpos + xpos, curypos + ypos);

                this.MotionManager().GetActualPos(EnumAxisConstants.Z, ref zActualpos);
                if (zclearence < zActualpos)
                {
                    this.MotionManager().AbsMove(axisZ,
                    zclearence,
                    axisZ.Param.Speed.Value,
                    axisZ.Param.Acceleration.Value);
                }

                if (xpos != 0)
                {
                    this.MotionManager().RelMoveAsync(axisX,
                        (diesizex),
                        axisX.Param.Speed.Value,
                        axisX.Param.Acceleration.Value);
                }

                if (ypos != 0)
                {
                    this.MotionManager().RelMoveAsync(axisY,
                      (diesizey),
                      axisY.Param.Speed.Value,
                      axisY.Param.Acceleration.Value);
                }

            }
            catch (Exception err)
            {
                LoggerManager.Exception(err);
                throw;
            }
            return Task.FromResult<int>(0);
        }

    }
}
