...
namespace RequestPinAlignBeforeCleaning
{
    public class RequestPinAlignBeforeNC : IProcessingModule, INotifyPropertyChanged
    {
        ...
        public RequestPinAlignBeforeNC()
        {

        }
        public RequestPinAlignBeforeNC(IStateModule Module)
        {
        ...
        public EventCodeEnum ClearData()
        {
            ...
            {
                LoggerManager.Debug($"{err.ToString()}. RequestPinAlignBeforeNC - ClearData() : Error occured.");
                throw err;
            }
            return retVal;
        }
        ...
        public EventCodeEnum DoExecute()         //실제 동작하는 logic
        {
            ...
            try
            {
                ...
                if (NeedleCleanModule.LotOPModule().InnerState.GetModuleState() == ModuleStateEnum.RUNNING)
                {
                    if (SubModuleState.GetState() != SubModuleStateEnum.SUSPEND)
                    {
                        ...
                        if (bFound == false)
                        {
                            // 클리닝은 하지 않는다. 즉 포커싱만 할 예정이니 핀 얼라인은 하지 않는다. 바로 넘긴다.
                        ...
                        if (NeedleCleanerParam.PinAlignBeforeCleaning.Value == true)
                        {
                            if (this.StageSupervisor().NCObject.PinAlignBeforeCleaningProcessed == false)
                            {
                                if (this.CommandManager().SetCommand<IDOPINALIGN>(this.NeedleCleaner()) == true)
                                {
                                    // 핀 얼라인 동작 요청
                                    LoggerManager.Debug("Request pin alignment before needle cleaning...");
                    ...
                    else
                    {
                        ...
                        if (this.StageSupervisor().NCObject.PinAlignBeforeCleaningProcessed == false)
                        {
                            ...
                            if (this.StageSupervisor().ProbeCardInfo.GetAlignState() == AlignStateEnum.DONE)
                            {
                                this.StageSupervisor().NCObject.PinAlignBeforeCleaningProcessed = true;
                                SubModuleState = new SubModuleDoneState(this);
                                RetVal = EventCodeEnum.NONE;
                            }
                            else
                        ...
                        else
                        {
                            ...
                            LoggerManager.Debug($"RequestPinAlignBeforeCleaning - Done flag does not initialized correctly");
                            SubModuleState = new SubModuleDoneState(this);
                            RetVal = EventCodeEnum.NONE;
                        }
                    }
            ...
            {
                LoggerManager.Debug($"{err.ToString()}. RequestPinAlignBeforeCleaning - DoExecute() : Error occured.");
        ...
        public EventCodeEnum DoClearData()
        {
            ...
            {
                LoggerManager.Debug($"{err.ToString()}. RequestPinAlignBeforeCleaning - DoClearData() : Error occured.");
                retVal = EventCodeEnum.UNKNOWN_EXCEPTION;
                throw err;
            }
            return retVal;
