...
{
    public class TOPFoupDockingPort : FoupDockingPortBase, ITemplateModule
    {
        ...
        public EventCodeEnum InitModule()
        {
            ...
            //{
            //    LoggerManager.Debug($"TOPFoupDockingPort.InitModule(): Init. error. Ret = {ret}");
            //}
        ...
        private TOPFoupDockingPortStateBase _State;

        public TOPFoupDockingPortStateBase State
        {
            get { return _State; }
            set { _State = value; }
        }

        public TOPFoupDockingPort(FoupModule module) : base(module)
        {
            StateInit();
        }

        public void StateTransition(TOPFoupDockingPortStateBase state)
        {
            try
            {
                _State = state;
        ...
        public override EventCodeEnum StateInit()
        {
            ...
            try
            {
                if (Module.DeviceParam.SubstrateSize.Value == SubstrateSizeEnum.INCH6)
                {
                    ...
                    if (ret != 0 || ret1 != 0)
                    {
                        StateTransition(new TOPFoupDockingPortStateError(this));
                    }
                    else
                    {
                        if (value == true && value2 == false)
                        {
                            StateTransition(new TOPFoupDockingPortStateIn(this));
                        }
                        else if (value == false && value2 == true)
                        {
                            StateTransition(new TOPFoupDockingPortStateOut(this));
                        }
                        else
                        {
                            StateTransition(new TOPFoupDockingPortStateError(this));
    ...
    public abstract class TOPFoupDockingPortStateBase
    {
        public TOPFoupDockingPortStateBase(TOPFoupDockingPort owner)
        {
            this.Owner = owner;
        }

        private TOPFoupDockingPort _Owner;
        public TOPFoupDockingPort Owner
        {
            get { return _Owner; }
            set
            {
                ...
                if (SubstrateSizeValid == true)
                {
                    ...
                    if (iobase != null)
                    {
                        if (iobase is ECATIOProvider)
                        {
                            ...
                            if (ret1 != 0 && ret2 != 0)
                            {
                                Owner.StateTransition(new TOPFoupDockingPortStateIn(Owner));
                            }
                            else
                            {
                                Owner.StateTransition(new TOPFoupDockingPortStateError(Owner));
                            }
                ...
                if (SubstrateSizeValid == true)
                {
                    ...
                    if (iobase != null)
                    {
                        if (iobase is ECATIOProvider)
                        {
                            ...
                            if (ret1 != 0 && ret2 != 0)
                            {
                                Owner.StateTransition(new TOPFoupDockingPortStateOut(Owner));
                            }
                            else
                            {
                                Owner.StateTransition(new TOPFoupDockingPortStateError(Owner));
    ...
    public class TOPFoupDockingPortStateIn : TOPFoupDockingPortStateBase
    {
        public TOPFoupDockingPortStateIn(TOPFoupDockingPort owner) : base(owner)
        {
        }
    ...
    public class TOPFoupDockingPortStateOut : TOPFoupDockingPortStateBase
    {
        public TOPFoupDockingPortStateOut(TOPFoupDockingPort owner) : base(owner)
        {
        }
    ...
    public class TOPFoupDockingPortStateError : TOPFoupDockingPortStateBase
    {
        public TOPFoupDockingPortStateError(TOPFoupDockingPort owner) : base(owner)
        {
        }
