...
namespace PinBlobThresholdAdvanceSetup.ViewModel
{
    using LogModule;
    ...
    public class BlobThresholdAdvanceSetupViewModel : IFactoryModule, INotifyPropertyChanged, IPnpAdvanceSetupViewModel
    {
        ...
        private async Task MaunalInputContolOKClick()
        {
            try
            {
                ...
                // 모드 변경에 따른 버튼 컨트롤 업데이트
                this.PnPManager().StepPageReload();
            }
            catch (Exception err)
            {
                LoggerManager.Exception(err);
